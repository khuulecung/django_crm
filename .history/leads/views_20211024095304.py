...
class CategoryListView(LoginRequiredMixin, generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_organisor:
            ...
            queryset = Lead.objects.filter(organisation=user.userprofile) # userprofile = test_admin who is an organisor
            category_set = Category.objects.filter(organisation=user.userprofile)
        elif user.is_agent:
            ...
            category_set = Category.objects.filter(organisation=user.agent.organisation)
        context.update({
            "unassigned_lead_count": queryset.filter(category__isnull=True).count(),
            "contacted_lead_count": category_set.filter(name="Contacted").count(),
            "contacted_lead_count": category_set.filter(name="Converted").count()
        })
